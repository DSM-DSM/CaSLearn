from stat_test.cit.diffusion.network import *
import torch
import torch.nn as nn
import warnings
from tqdm import tqdm
from stat_test.cit.diffusion.model import *
from stat_test.cit.diffusion.nnlscit import *


def perform_diffusion_crt(xxx, yyy, zzz, xxx_crt, yyy_crt, zzz_crt,
                          repeat=100, device: torch.device or str = torch.device('cpu'),
                          verbose=False, seed=None, stat='cmi',
                          centralize=False, sampling_model='ddpm', num_steps = 1000):
    '''
    xxx,yyy,zzz: triple used to train diffusion model, learning Y|Z. and when computing CMI, we use 1-nn to learn X|Z. 
    you can see that in line 49,136 in nnlscit.py. in our paper, we wrote using diffusion model to learn X|Z and when
    computing CMI, we use 1-nn to learn Y|Z.
    but that doesn't effect the outcome because in terms of R.V., X and Y is changeable. 
    actually, we also tried to use both diffusion model and 1-nn to learn Y|Z (or X|Z), which will have a little bit
    worse and unstable performance.
    ...
    For unstructural data, you can use a VAE to easily convert it into vectors, and use our CMI statistics, or just find
    some neural maps T(X,Y,Z) to conduct CRT.
    
    xxx_crt,yyy_crt,zzz_crt: triple used to compute T(X,Y,Z).
    
    for your own dataset (X,Y,Z), you need to randomly split it into 2 parts (xxx,yyy,zzz) and
    (xxx_crt,yyy_crt,zzz_crt). we use equal X_num size.
    ...
    stat: the defalut is 'cmi': conditional mutual information (CMI), we also provided 'corr': pearson corr, 'rdc':
    ...
    note that we highly recommend users to use sampling_model='ddpm', because ddpm can provide smoother forward process
    and reverse process, thus outputing better results. sampling_model='ddpm' is especially better
     when X,Y,Z are high dimensional.
    '''
    ...
    for z in range(num_epoch):
        ...
        for idx, batch in enumerate(dataloader):
            ...
            if sampling_model == 'score':
                ...
                loss = diffusion_loss_fn(model, batch_y, batch_x, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, num_steps,
                                         device)
            optimizer.zero_grad()
            loss.backward()
    ...
    if stat == 'cmi':
        ...
        original = NNCMI(xxx_crt, yyy_crt, zzz_crt, 1, 1, dz,
                         # if you want to try high-dim x,y, don't forgot to change here
        ...
        with torch.no_grad():
            ...
            if sampling_model == 'score':
                ...
                y_seq_crt = sample_from_diff(model, num_samples=zzz_crt.shape[0],
                                             ...
                                             device=device)
        if stat == 'cmi':
            # compute T(x,pseudo_y,z). again, we note that when computing CMI, we use 1-nn to learn x|z,
            # which is equivalent with our paper. see nnlscit.py
