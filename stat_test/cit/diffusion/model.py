...
def sample_from_diff(model, num_samples, input_dim, cond, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, betas, num_steps,
                     device):
    traj = []
    ...
    for t in reversed(range(num_steps)):
        ...
        if t > 0:
            ...
            x_t = (1 / alpha_t_sqrt) * (x_t - (1 - alpha_t) / one_minus_alphas_bar_sqrt[t] * noise_pred) + torch.sqrt(
                beta_t) * noise
            traj.append(x_t)
        else:
            alpha_t = 1 - betas[t]
